    ...
    <section class="">
        <div class="mb-2 row">
            <div class="col-sm-12 col-md-12 col-lg-12">
                <a href="/management/property/create">
                            ...
                            <tbody>
                                @foreach ($responseBody->data as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->landlord_id ? $item->landlord->first_name . ' ' . $item->landlord->last_name : 'HIG' }}</td>
                                        <td>{!! $item->is_leased === 1 ? '<span class="badge badge-success">LEASED</span>' :
                                            '<span class="badge badge-warning">EMPTY</span>' !!}</td>
                                        <td>{{ $item->is_listed === 1 ? 'YES' : 'NO' }}</td>
                                        <td>
                                            {!!  
                                                $item->is_listed ? 
                                                '
                                                <a href="/management/property/'.$item->id.'/unpublish">
                                                    <button class="mr-1 mb-1 round btn btn-outline-danger">Unpublish</button>
                                                </a>
                                                ':
                                                '
                                                <a href="/management/property/'.$item->id.'/publish">
                                                    <button class="mr-1 mb-1 round btn btn-outline-info">Publish</button>
                                                </a>
                                                ' 
                                            !!}
                                            <a href="/management/property/{{ $item->id }}/edit">
                                                <button class="mr-1 mb-1 round btn btn-outline-warning">Edit</button>
                                            </a>
                                            <a href="/management/property/{{ $item->id }}/delete">
